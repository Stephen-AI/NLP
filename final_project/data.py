...
def get_qid_entities(qid: str, strs: List[str], is_passage=True):
    pass_dict = entity_dict[qid]
    ents = [0] * len(strs)
    pos_to_idx = {0: 0}
    prev_pos = 0
...
class QADataset(Dataset):
    ...
    def _create_data_generator(self, shuffle_examples=False):
        ...
        for idx in example_idxs:
            ...
            # ADDED
            p_ents = torch.tensor(
                get_qid_entities(qid, passage)
            )
            q_ents = torch.tensor(
                get_qid_entities(qid, question, False)
            )
            ...
            passage_entities.append(p_ents)
            question_entities.append(q_ents)
        ...
        return zip(passages, questions, start_positions, end_positions, 
                   passage_entities, question_entities)
    ...
    def _create_batches(self, generator, batch_size):
        ...
        while True:
            ...
            for ii in range(bsz):
                ...
                # ADDED
                psg_entities[ii].append(current_batch[ii][4])
                qst_entities[ii].append(current_batch[ii][5])
